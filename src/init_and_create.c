...
/*   Updated: 2023/06/05 01:27:58 by mbelouar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_threads(t_philo *philo, pthread_mutex_t	*forks)
{
	...
	initialize_philos(all_philos, philo, forks);
	i = -1;
	while (++i < philo->nb_philo)
	{
		if (pthread_create(&philo->thread_id[i], NULL,
				routine, &all_philos[i]) != 0)
			return (free (philo->thread_id), 1);
	...
	while (1)
	{
		...
		while (++i < philo->nb_philo)
		{
			...
			if (ft_time() - all_philos[i].time > philo->time_to_die)
			{
				...
				printf("%ld %d is died\n", ft_time() - all_philos[i].start_time,
					all_philos[i].philo_id);
				pthread_mutex_unlock(philo->eat_time);
				return (1);
			}
			if (check_meals(philo, all_philos, i) == 1)
				return (1);
			pthread_mutex_unlock(philo->eat_time);
			usleep(100);
		}
...
{
	if (all_philos[i].meal >= philo->nb_philo * philo->nb_meals
		&& philo->nb_meals)
	{
		pthread_mutex_unlock(philo->eat_time);
		return (1);
			...
			pthread_mutex_t *forks)
{
	...
	while (++i < philo->nb_philo)
	{
		...
		all_philos[i].time_to_die = philo->time_to_die;
